...
import datetime
def generar_id():
    return str(random.randint(1000, 9999)for _ in range (4))  # Número aleatorio de 4 dígitos
#tarea vacia 
tareas = []
def cargar_tareas():
    try:
...
def crear_tarea():
    ...
    categoria = input("Ingrese la categoria (pendiente, en progreso, completada): ")
...
def actualizar_tarea():
    ...
    tarea_id = int(input("Ingrese el ID de la tarea que desea actualizar: "))  # Corregido
...
def recordar_tarea():
    ...
    for tarea in tareas:
        try:
            fecha_tarea = datetime.datetime.strptime(tarea[2], "%d/%m/%Y").date()  # Convertir limite fecha a objeto date
            if 0 <= (fecha_tarea - dia_actual).days <= 3 and tarea[5].lower() != "completada": #calcula la fecha limite con la actual y si no esta completada mostrara el recordatorio
                print(f"Recordatorio - ID: {tarea[0]}, Descripción: {tarea[1]}, Fecha Límite: {tarea[2]}") #imprime el recordatorio
        except ValueError:
